...
// Instance Variables
private TellerServerClientInterface server;
...
public TellerClient(String serverAddress) throws Exception // CONSTRUCTOR
  ...
  server = (TellerServerClientInterface) Naming.lookup("rmi://" + serverAddress + "/BankServer");
...
private void createNewAccount(String accountType) throws Exception
{
	...
	String serverReply = server.createNewAccount(accountType, getCustomerName("createNewAccount"));
...
private void showAccount() throws Exception
{
	...
	if(getAccountNumber("showAccount") == 0)
	{
		
	String serverReply2 = server.showAccount(getAccountNumber("showAccount"), getShortCustomerName("showAccount"));
	displayTextArea.setText(serverReply2);
	}
	else
	{
	String serverReply = server.showAccount(getAccountNumber("showAccount"), getCustomerName("showAccount"));
	displayTextArea.setText(serverReply);
	}
}
//*******************************************************************************
...
{
	String serverReply = server.processAccount(processingType, getAccountNumber("processingType"), getAmount("processingType"),getCustomerName("processingType") );
	displayTextArea.setText(serverReply);
	if (!serverReply.startsWith("ERROR:"))
	{  transactionLogTextArea.append(newLine + serverReply);
...
private void closeOutAccount() throws Exception
{
	...
	String serverReply = server.closeOutAccount(getAccountNumber("closeOutAccount"), getCustomerName("closeOutAccount"));
	displayTextArea.setText(serverReply);
	if (!serverReply.startsWith("ERROR:"))
	    transactionLogTextArea.append(newLine + serverReply);
